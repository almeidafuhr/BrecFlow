...
public class FrmEntrada extends JFrame {
    ...
    public FrmEntrada(String user) {
        super("BrecFlow  -->  Entrada de itens");
        initComponents();
    ...
    private void BTatualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BTatualizarActionPerformed
        ...
        if(RBid.isSelected()){
            sqlUpdate = "UPDATE ITEM SET Qtd_Item = Qtd_Item + ? WHERE ID_ITEM = ?;";
            id = Integer.parseInt(TFbusca.getText());
        }
        if(RBreferencia.isSelected()){
            sqlUpdate = "UPDATE ITEM SET Qtd_Item = Qtd_Item + ? WHERE Referencia_Item = ?;";
        ...
        try{
            ...
            if(resultado == 1){  //Se houve uma execução
                cadastrarEntrada(conexao, id);  //Cadastra um registro na tabela Entrada enviando a conexão e o id
                try {
                    log.generateLog("Usuário " + user + " Atualizou o item de id " + id + ", gerando entrada de " + quantidade + " itens no estoque.");
        ...
        if(TFbusca.getText().equals("")){
            ...
            if(RBid.isSelected()){
                try{
                    ...
                    JOptionPane.showMessageDialog(null, "Valor inválido para campo numérico!");
                    TFbusca.setEnabled(true);
                    TFbusca.setText("");
                    TFbusca.grabFocus();
                }
    ...
    private void cadastrarEntrada(ConexaoSQLite conexao, int id_item){
        ...
        try{
            ...
            if(resultado == 1){
                JOptionPane.showMessageDialog(null, "Entrada gerada com Sucesso");
            }else{
